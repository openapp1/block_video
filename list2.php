...
$PAGE->set_url('/block/video/list.php');
$PAGE->navbar->add(get_string('pluginname', 'block_video'), new moodle_url('/block/video/'));
$PAGE->navbar->add(get_string('list', 'block_video'));
...
$PAGE->requires->js('/blocks/video/video_list.js');
...
//     $message = get_string('nostreaming', 'block_video');
...
$f = 'selected, thumb, id, name, usergroup, orig_filename, length, timecreated';
$fields = $f;
// echo "<pre>";
// var_dump($fields);
...
foreach ($fields as $key => $value) {
    ...
    if (!(
        ...
        ($settings->embedcolumn == 0 && trim($value) == 'streaming_url')
            ...
            array_push($liststrings, get_string(trim($value), 'block_video'));
    }
}
// var_dump(array_values($fieldsv));die;
// var_dump($liststrings);die;
...
// }
// 'alltags' => $alltagsurl,
//   'existvideotags' => is_array($SESSION->video_tags),
//  'videotags' => $selectedtags,
// showgroupcloud' => $settings->groupcloud, 
//  'groups' => array_values($g),
//  'existselectedgroups' => count($SESSION->groups), 
//  'selectedgroups' => array_values($SESSION->groups),
//  'showcatscloud' => $settings->catscloud, 
//  'categories' => array_values($cats),
//  'existselectedcats' => count($SESSION->categories), 
//  'selectedcats' => array_values($selectedcats),
//  'tags' => $tags, 

echo $OUTPUT->render_from_template('block_video/list2',
 ['wwwroot' => $CFG->wwwroot,

 'liststrings' => $liststrings,
 'lang' => current_language(),
 'fields' => array_values($fieldsv),
 'order' => $order
 ]);
